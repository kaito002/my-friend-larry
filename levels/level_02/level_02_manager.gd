...
func _ready():
	...
	pause_menu.connect('menu_showed', func(): Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE))
	pause_menu.connect('menu_hided', func(): Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED))
	pause_menu.connect('restart', _restart_level)
